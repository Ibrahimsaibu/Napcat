...
export const RichTextComponents = {
  ...
  list: {
    bullet: ({ children }: any) => (
      <ul className="ml-10 py-5 list-disc space-y-5 list-inside">{children}</ul>
    ),
    number: ({ children }: any) => (
  ...
  block: {
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="border-l-[#7000ff] bprder-l-4 pl-5 py-5 my-5">
        {children}
      </blockquote>
    ),
  },
    ...
    link: ({ children, value }: any) => {
      ...
      return (
        <Link
          href={value.href}
          rel={rel}
          className="underline decoration-[#7000ff] hover:decoration-black"
        >
          {children}
        </Link>
      );
